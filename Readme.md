...
change the ball collection radius as the level grows
...
balls keep their place ? or flip too ? or keep their momentum ?
...
Set a low max when the game start
Every 5 seconds, update the max, based on FPS, render time, coins count
Measure render time
If max reached, render time is good and fps are good, raise max
If fps drops, lower the max
...
### Re-introduce re-rolls 🟨

medals give rerolls
10 re-rolls let you loop and play 7 more levels. 
rerolls can be use to increase last level's score

### max 7 upgrades, each level up gives only one upgrade 🟨
