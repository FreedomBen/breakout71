...
export async function openUpgradeDetails(id: Upgrade["id"], onClose:()=>void) {
  const {name, help, fullHelp, gift} = upgrades.find(u => u.id === id) as Upgrade

  const allowedAsStart = getSettingValue(
    "start_with_" + id,
    gift,
  );
  const allowedInGame = getSettingValue(
    "offer-upgrade-" + id,
    true,
  );

  const allowDisabling = !allowedInGame|| upgrades
  ...
  const action = await asyncAlert<string>({
    ...
    content: [
      ...
      },
      {
        ...
        help: allowDisabling ? t('unlocks.upgrade_choice_perk_help') : t('unlocks.upgrade_choice_perk_locked'),
